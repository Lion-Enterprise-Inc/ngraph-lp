...
const shisan = P.find(p => p[3] === '試算');
// 試算の県は答申が出そろえば0になる（2026-09-16に0件になった）。
// 「1つ以上ある」を条件にすると、その日から検査が落ちるだけで何も守らない。
// 守るべき性質は「ラベルが3種のどれかであること」なので、そちらを全県で見る。
expect('ラベルは決定/答申/試算のどれか（47県）', P.every(p => ['決定','答申','試算'].includes(p[3])), P.filter(p => !['決定','答申','試算'].includes(p[3])).map(p => p[0]+':'+p[3]).join(','));
