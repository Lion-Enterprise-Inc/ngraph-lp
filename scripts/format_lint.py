...
RULES = {
    ...
    "夕": (6000, None, 7, None),
    "ガイド": (3000, 8000, 5, 10),
    # ナレッジ型（2026-08-15新設・KNOWLEDGE-OPS.md）: 社内ナレッジ発信シリーズ（案件直結）。
    # 週1の別ルーティン。実録＋手順が本体なので夕Bほどの厚み要件は課さない
    "ナレッジ": (3000, 8000, 5, None),
}
TARGET = {"朝": "2,500〜3,500字・H2 4本（BLOG-OPS §2）",
          "夕": "6,000〜8,000字・H2 7〜9本（BLOG-OPS §3）",
          "ガイド": "3,000〜8,000字・H2 5〜10本（BLOG-OPS §3.5）",
          "ナレッジ": "3,000〜8,000字・H2 5本以上（KNOWLEDGE-OPS.md）"}
# 上限を超えたときに何が壊れるかは型ごとに違う。使い回すと理由が嘘になる
OVER = {"朝": "速報性が死ぬ",
        "ガイド": "使い方の材料が尽きて水増しになる。深掘りなら夕B型で書く"}
# 定時の枠。1日1本ずつしか無い。ガイド型は枠を消費しない3本目
SLOTS = ("朝", "夕")
LABEL_RE = r"(20\d\d)\.(\d\d)\.(\d\d)\s*(朝|夕|ガイド|ナレッジ)"
