...
ルール（BLOG-OPS §0）:
- 原価Excelの件は無条件NG（2026-09-22。上の「公表可能な実話」からこの1点を外す。
  AIが顧客のExcelを見て課題と呼んだだけで、顧客も髙橋さんも課題と言っていない）
- 3点セットを1記事に2つ以上入れない
...
def main():
    ...
    # 原価Excelの件は「使い回し」以前に書いてはいけない（2026-09-22 髙橋さん）。
    # AIが顧客のExcelを見て勝手に「課題」と呼んだだけで、顧客も本人も課題と言っていない。
    if "原価Excel" in mine:
        print("NG: 原価Excelの件は使用禁止 — 顧客も本人も課題と言っていない。削る")
        ok = False
